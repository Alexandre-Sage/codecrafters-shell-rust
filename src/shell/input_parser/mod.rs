...
pub struct ParsedCommand {
    command: String,
    args: Vec<String>,
    redirection_context: Option<RedirectionContext>,
}
...
impl ParsedCommand {
    ...
    pub fn redirection(&self) -> &Option<RedirectionContext> {
        &self.redirection_context
    }
}
...
impl InputParser {
    ...
    pub fn parse(&self, input: &str) -> Result<ParsedCommand, CommandError> {
        ...
        let maybe_redirection = self.parse_redirection(&parsed_args)?;

        let command = &parsed_args[0];
        Ok(ParsedCommand::new(
            command,
            parsed_args[1..].to_vec(),
            maybe_redirection,
        ))
    }
}
...
mod tests {
    ...
    fn parse_simple_output_redirection() {
        ...
        assert!(parsed.redirection().is_some());
        let redir = parsed.redirection().as_ref().unwrap();
        assert_eq!(redir.path, PathBuf::from("output.txt"));
        assert_eq!(redir.redirection_type, RedirectionType::Output);
    }
